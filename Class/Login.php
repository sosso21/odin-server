...
*/
    ...
    class Login
    {
    	...
    	public 	function issub()
    	{
    		...
    		$request2 = $pdo->prepare("SELECT COUNT(*) FROM user WHERE (email = ? or id = ? ) and (password = SHA2( ? , 256) or password = ? )  and isActif = 1 ");
        ...
        public function Info()
        {
                 require(ROOTPATH.'elements'.DIRECTORY_SEPARATOR.'db_config.php');
            $pdo  = new PDO($db_DSN , $db_USER , $db_PASS ) ;
            $request3 = $pdo->prepare("SELECT * FROM user WHERE (email = ? or id = ? )  and (password = SHA2( ? , 256)   or password = ? ) and isActif= 1");
